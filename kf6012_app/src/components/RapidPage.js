...
*
* This is the page for the rapid track, this returns information about rapid papers
*
* @author Martyn Clow W20045942
*/

function RapidPage() {
    const [rapidPapers, setRapidPapers] = useState([]);
    const [selectPaper, setSelectPaper] = useState('all');
    const [loading, setLoading] = useState(true);
    const [searchTerm, setSearchTerm] = useState('');

    useEffect( () => {
        fetch("http://unn-w20045942.newnumyspace.co.uk/kf6012/assignment/api/paper?track=rapid")
        .then(
            (response) => response.json()
        )
        .then(
            (json) => {setRapidPapers(json.data)
            setLoading(false)}
        )
        .catch((err) => {
...
const listOfRapidPapers = (
    <section className='papers'>
        {rapidPapers.filter(searchPapers).filter(selectPapers).map(
            (value, key) => (
